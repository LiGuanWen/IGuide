...
- (void)showGuideAtIndex:(NSUInteger)index {
    ...
    UIBezierPath *annotatedPath = [UIBezierPath bezierPathWithRoundedRect:highlightFrameOfAnnotated cornerRadius:item.cornerRadiusOfAnnotated];
    UIBezierPath *toPath = ({
        UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRect:self.view.bounds];
        [bezierPath appendPath:annotatedPath];
        bezierPath;
    });
...
- (UIButton *)skipButton {
    if (!_skipButton) {
        ...
        button.alpha = 0.0;
        button.safeInsets = UIEdgeInsetsMake(0, 20, 0, 20);
